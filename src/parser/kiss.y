...
%start program
%type<nodeVal> statementList statement expression precedentExpression compoundStatement varDefInitList varDefList parameterList nonEmptyParameterList argumentList nonEmptyArgumentList constant

%%

program:
	statementList	{ program = new AProgram(yyLineNo, $1); }
	| 				{ program = new EmptyProgram(yyLineNo); }
	;

statementList:
	statement					{ $$ = new OneStatementSL(yyLineNo, $1); }
	| statement statementList	{ $$ = new MulStatementSL(yyLineNo, $1, $2); }
	;

statement:
	expression ';'										{ $$ = new ExpressionS(yyLineNo, $1); }
